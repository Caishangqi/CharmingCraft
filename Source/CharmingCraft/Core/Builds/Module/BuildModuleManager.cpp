...
bool UBuildModuleManager::StartBuildPreviewTrace(UItemStack* PreviewItemStack, ACharacter* Instigator)
{
	...
	if (PreviewItemStack)
	{
		...
		// To put in the world
		BlockEntityActor = Cast<UBlockMeta>(PreviewItemStack->ItemMeta)->CreateBlockEntityActor(this,GameInstance->GetWorldManager()->GetShownWorld()->GetGamePlayWorldInstance()->GetLevelScriptActor());
	}
