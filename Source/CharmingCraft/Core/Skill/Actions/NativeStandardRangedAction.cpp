...
FVector UNativeStandardRangedAction::GetCastMouseLocation(APawn* Instigator)
{
	...
	ObjectType.AddObjectTypesToQuery(ECollisionChannel::ECC_WorldDynamic);
	...
	if (PlayerController->DeprojectMousePositionToWorld(WorldLocation, WorldDirection))
	{
		...
		DrawDebugLine(GetWorld(), CameraLocation, End, FColor::Red, false, 2.0f, 0, 4);
		if (bHit)
		{
			for (FHitResult OutHit : OutHits)
			{
				...
				UE_LOG(LogTemp, Display, TEXT("Get Mouse Location at Actor  <%s>"), *OutHit.GetActor()->GetName());
